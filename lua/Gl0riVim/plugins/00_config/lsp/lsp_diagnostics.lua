...
if not settings_status then
  vim.notify("[settings] failed to load within (lspconfig.lua)", vim.log.levels.WARN)
  return
end

local icons_status, icons = pcall(require, "icons")
if not icons_status then
  vim.notify("[icons] failed to load within (lsp_diagnostics.lua)", vim.log.levels.WARN)
  return
end
...
if not lspui_ok then
  vim.notify("[lsp.ui] failed to load within (lsp_diagnostics.lua)", vim.log.levels.WARN)
  return
end
...
sign({ name = "DiagnosticSignWarn", text = icons.diagnostics.Warn })
