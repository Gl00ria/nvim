return {
  ...
  config = function()
    ...
    local keymap = {
      ["<C-u>"] = function()
        neoscroll_.ctrl_u({ duration = 250 })
      end,
      ["<C-d>"] = function()
        neoscroll_.ctrl_d({ duration = 250 })
      end,
      ["<C-b>"] = function()
        neoscroll_.ctrl_b({ duration = 450 })
      end,
      ["<C-f>"] = function()
        neoscroll_.ctrl_f({ duration = 450 })
      end,
      ["<C-y>"] = function()
        neoscroll_.scroll(-0.1, { move_cursor = false, duration = 100 })
      end,
      ["<C-e>"] = function()
        neoscroll_.scroll(0.1, { move_cursor = false, duration = 100 })
      end,
      ["zt"] = function()
        neoscroll_.zt({ half_win_duration = 250 })
      end,
      ["zz"] = function()
        neoscroll_.zz({ half_win_duration = 250 })
      end,
      ["zb"] = function()
        neoscroll_.zb({ half_win_duration = 250 })
      end,
    }
    local modes = { "n", "v", "x" }
